...
func setUpBackButton() *gtk.Box {
	...
	pixbuf, _ := createPixbuf("go-previous", *iconSizeLarge)
...
func setUpUserDirsList() *gtk.ListBox {
	...
	row := setUpUserDirsListRow("user-home", "Home", "home", userDirsMap)
	listBox.Add(row)
	row = setUpUserDirsListRow("folder-documents", "", "documents", userDirsMap)
	listBox.Add(row)
	row = setUpUserDirsListRow("folder-download", "", "downloads", userDirsMap)
	listBox.Add(row)
